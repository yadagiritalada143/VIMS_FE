...
export class NewFlowComponent implements OnInit {
  ...
  removeLevel(levelIndex) {
    ...
    this.allSelectedRecipientsData = this.level_group.rootLevel.map(x => x.recipients)?.map(y => y.recipient_type_id)
  }
  ...
  onRecipientLevelDataChange(recipientData,levelIndex) {
    ...
    this.allSelectedRecipientsData = this.level_group?.rootLevel?.map(x => x?.selectedrecipients)?.map(y => y?.recipient_type_id)
  }
  ...
  loadFlowConfigData(flowConfigObj) {
    ...
    if(this.level_group.rootLevel.length>0) {
  ...
  outerLevelRemoveGroup(levelIndex, outerIndex) {
    this.level_group.rootLevel[levelIndex].levelConditions.splice(outerIndex+1,1)
  }
  ...
  createFlow() {
    ...
    if(payload.levels) {
      for(let i=0; i<payload?.levels.length; i++) {
        ...
        if(payload.levels[i].placement_order != 0 && (!payload.levels[i].recipient_types.length || payload.levels[i].recipient_types.length == 0 || !payload.levels[i]?.recipient_types[0]?.recipient_type_id)) {
    ...
    this.flow_id ? this.progServ.put(`/configurator/programs/${this.programDetails?.id}/flow-configs/${this.flow_id}`, payload).subscribe(
      data => {
        if(data) {
          ...
          this.Router.navigate(['flows-management']);
        }
      },
      err => {
