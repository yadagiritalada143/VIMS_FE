...
@Component({
  selector: 'app-create-rate-factor-list',
  templateUrl: './create-rate-factor-list.component.html',
  styleUrls: ['./create-rate-factor-list.component.scss']
})
export class CreateRateFactorListComponent implements OnInit {
  ...
  constructor (
    ...
    private camelCasePipe: TitleCasePipe,
  ...
  get getPercentageLabel() {
    let val: string | null = this.rateFactorForm.get('rate_factor_type').value;
    if (val === null)
      return 'Percentage on ';
    else
      val = this.camelCasePipe.transform(val.toLowerCase().replace('_', ' '));
    return "Percentage on " + val + " ";
  }

  getWidth(val: string) {
    let formula = (val.length <= 6) ? (val.length * 16) : (96 + (8 * (val.length - 6)));
    return { "width": Math.max(formula, 80) + 'px' };
  }
