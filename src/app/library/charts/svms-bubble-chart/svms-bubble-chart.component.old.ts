...
/*
      const modifiedData = data.map(element => {
        ...
        elemToSet.name = element[this.label];
        elemToSet.size = element[this.value];
        return elemToSet;
      });
      this.data = {
        name: 'flare',
  ...
  drawChart() {
  //  if (!this?.data?.children[0]?.children?.length) {
  //    return;
  //  }
    d3.select('.bubble').remove();
  ...
  private drawBubbles() {
            ...
            .on('mouseover', function(e, data) {
              ...
              const table = that.tooltip.selectAll('table')
                ...
                .join(
                  enter => {
                    enter.append('table')
                    .append('tr').append('th').attr('colspan', 3).html(that.titleCase(that.value));
                  },
                  update => { update.select('th').html(that.titleCase(that.value)); }
                );
              const rows = [data];
              ...
              content.join(
                enter => {
                  ...
                  r.append('td').classed('name', true)
                    .text(d => that.titleCase(d[that.label]));
    ...
    .range([this.chartWidth, 0])
      .domain(this?.data.children[0]?.children.map(item => item.size));
    ...
    this.svg.append('text')
      .attr('transform', `translate(${this.chartWidth / 2}, ${this.chartHeight + 30})`)
      .style('text-anchor', 'middle')
      .style('fill', '#7F92A3')
      .style('font-size', '12px')
      .text(this.label);

    const maxValue = Math.max(...this.data?.children[0]?.children.map(item => item.size));

    this.svg.selectAll('.node')
      .data(this.data)
      .attr('class', 'node');

    const diameter = this.chartWidth / 3;
    this.color = d3.scaleOrdinal(d3.schemePaired);
    ...
    const root = d3.hierarchy(classes(this.data))
      .sum(d => d['value']);

    bubble(root);
    const node = this.svg.selectAll('.node')
      .data(root.children.map(item => {
        return {
          ...item,
          y: this.chartHeight - (this.chartHeight * item.value / maxValue),
        };
      }))
      .enter().append('g')
      .attr('class', 'node')
      .attr('transform', d => {
        const transformX = d?.x || 0;
        const transformY = d?.y || 0;
      ...
      .on('mouseenter', (d, i) => {
        this.tooltip
          ...
          .style('left', i?.x + 'px')
          .style('top',  i?.y - this.chartHeight + 'px')
          .style('opacity', 1)
          .html(`${i?.data?.className}: ${i?.value}`);
      }).on('mousemove', (d, i) => {
        this.tooltip
          .style('left', i?.x + 'px')
          .style('top',  i?.y - this.chartHeight + 'px');
    ...
    function classes(root) {
      ...
      function recurse(name, node) {
        if (node?.children){
          ...
          classesArray.push({ packageName: name, className: node?.name, value: node?.size });
        }
      }
