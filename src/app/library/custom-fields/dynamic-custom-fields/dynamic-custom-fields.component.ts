...
export class dynamicCustomFieldsComponent implements OnInit , AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes?.hierarchyIds?.previousValue && changes?.hierarchyIds?.previousValue !== changes?.hierarchyIds?.currentValue)
      this.ngOnInit();
  }
      ...
      this.customFieldsForm?.get('custom_fields')['controls']?.forEach(control => {
        if (control?.value?.is_show) {
          ...
          if (control.value.custom_field_type == this.customFieldsEnum.NUMBERS) {
            control.value.values = parseFloat(control.value.values?.toString().replace(/,/g, ''));
  ...
  get customFieldsArray() {
    if((this.customFieldsForm?.get('custom_fields') as UntypedFormArray || new UntypedFormArray([]))?.status?.toUpperCase() === 'VALID'){
        ...
        for (const [key, value] of Object.entries(custom_fields)) {
          if (element?.value?.custom_field_slug === key) {
            if (element?.value?.custom_field_type?.toUpperCase() === this.customFieldsEnum.DATE) {
              ...
              const date = this.datePipe.transform(value, null, null, null, true);
                element?.get('date')?.patchValue(date)
              ...
              element
                ...
                const time = (value as string)?.split(" ")?.[1];
              ...
              if (element?.value?.meta_data?.show_in_thousands || element?.value?.meta_data?.decimal) {
                this.getCommaSaparatedValue(value ? value : null, element?.value?.custom_field_id)
              } else {
                element.get('values').patchValue(value);
 ...
 mapDependentFields(dependentFields) {
  ...
  updateCustomFieldForm() {
    ...
    this.customFields?.forEach((element ,i) => {
      ...
      const isElementSingleRow = ((element?.type == this.customFieldsEnum.CHECKBOX || element?.type == this.customFieldsEnum.RADIO || element?.meta_data?.datasource?.is_multi_select || element?.type == this.customFieldsEnum.MULTI_SELECT_DROPDOWN || (element?.type == this.customFieldsEnum.PICKLIST && element?.pick_list?.multiselect) || element?.type == this.customFieldsEnum.FILE || element?.type == this.customFieldsEnum.IMAGE)) ? true : false;
      this.customFieldsArray.push(
        this.fb.group({
          ...
          date: [element?.type == this.customFieldsEnum.DATE ? this.getValue(element) : null]
        })
      )
      if(element?.type == this.customFieldsEnum.NUMBERS) {
        if (element?.meta_data?.show_in_thousands || element?.value?.meta_data?.decimal)
          this.getCommaSaparatedValue(element?.meta_data?.default_value ? element?.meta_data?.default_value : null, element.id);
        else
  ...
  onDateChange(element) {
    const dateVal = this.datePipe.transform(element?.get('date')?.value, this._dateFormat, null, null, true, this.program.defaultDateFormat);
    element?.get('values')?.patchValue(dateVal);
  }
  
  colsInRow() {
    return 'col-' + Math.round(12/this.columnInRow);
  }
