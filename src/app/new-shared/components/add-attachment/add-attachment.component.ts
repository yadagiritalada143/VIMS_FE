    ...
    if (event && (event?.target?.files?.length || event?.dataTransfer?.files?.length)) {
      ...
      if (this.isValidFileFormat(fileExtension)) {
        ...
        if (this.fileSize <= this.maxFileSize) {
          ...
          upload['is_vendor_allowed_to_view'] = true;
              ...
              uploadRes => {
                if (uploadRes.type === HttpEventType.Response) {
                  ...
                  if (this._multiple) {
                    ...
                    this.uploadedFileArr.forEach(res => {
                      const data = { ...res };
                      this.formattedUploadArr.push({
                        ...
                        time: data['time'],
                        size: data['size'],
                        is_vendor_allowed_to_view: data['is_vendor_allowed_to_view']
                      })
                    })
                  } else {
                    this.uploadedFileArr = [upload];
                    this.formattedUploadArr = [
                      { name: upload['name'], key: upload['key'], ext: fileExtension, time: upload['time'], size: this.fileSize, is_vendor_allowed_to_view: true },
                    ];
                  }
          ...
          this.alert.error(`File size must be less than ${this.maxFileSize} MB`)
          this.afterUpload();
        }
      } else {
