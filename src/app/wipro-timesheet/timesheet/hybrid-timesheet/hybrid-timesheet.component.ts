...
@Component({
  selector: 'app-hybrid-timesheet',
  templateUrl: './hybrid-timesheet.component.html',
  styleUrls: ['./hybrid-timesheet.component.scss']
})
export class HybridTimesheetComponent implements OnInit, OnDestroy {
  dateFormatEnum = DATE_FORMAT; 
  selectedCustomData = {};
  customFields: any[] = [];
  isCustomFieldsValid: any;
  public timesheetTabEnum = timesheettabenum;
    ...
    this.subscriptions.push(this.timesheetService.getProject(currentTimesheetData?.assignment_id, projectType, value).subscribe(
      {
        next: (data: any) => {
          data?.data?.forEach(projectCode => {
            projectCode.originalCode = projectCode?.rate_factor || projectCode?.code;
          ...
          data?.data?.forEach(pr => {
            pr.project_code = pr?.rate_factor;
            pr.project_type = pr?.type || pr?.parent_type;
          });
    ...
    this.subscriptions.push(this.timesheetService.getAccountCodes(this.currentTimesheetData?.assignment_id).subscribe(
      {
        next: (data: any) => {
          data?.data?.forEach(projectCode => {
            projectCode.originalCode = projectCode?.rate_factor || projectCode?.code;
            projectCode.project_type = projectCode?.parent_type || projectCode?.type;
            const show_account_code = this.timesheetService.showOnlyCodes();
            if (show_account_code) {
              projectCode.code = projectCode?.code || projectCode?.account_code || projectCode?.title;
            } else {
  ...
  isPresentCode() {
    ...
    this.formData?.rowData?.forEach((row: any) => {
      this.dropDownItems?.forEach((data: any) => {
        if ((row?.title == data?.project_title) && (row?.projectCode == data?.projectCode)) {
  ...
  setProjectValues(col) {
    ...
    if (option) {
      col.project_code = option?.rate_factor;
    ...
    if (this.projectCodes?.length > 0) {
      ...
      for (let i = 0; i < this.projectCodes?.length; i++) {
        if (!this.projectCodes[i]?.start_date || (getDateFromString(this.projectCodes[i]?.start_date) <= timesheetEndDate)) {
          this.dropDownItems.push({ "display": count, ...this.projectCodes[i], "projectCode": this.projectCodes[i].code + '(' + this.projectCodes[i].rate_factor + ')', "isSelected": false, ...this.projectCodes[i]});
          count = count + 1;
          if (this.ruleConfiguration?.overtime?.overtime?.is_allow && (this.config?.hour_type_calculation === 'hybrid' && this.projectCodes[i]?.parent_type != 'project-x')) {
  ...
  public updateDropDown(val) {
    if (val) {
      ...
      if(val?.project_title === 'default' || val?.project_type === 'default' 
      || (val?.projectInfo?.code === 'null(default)' || val?.projectInfo?.projectCode === 'null(default)' 
      || (val?.projectInfo?.originalCode === 'default')) 
      || (!val?.projectInfo?.code && !val?.projectInfo?.projectCode && !val?.projectInfo?.originalCode)){
        val.project_code = null;
        val.project_title = null;
        val.project_type = "default"; // Need to get dynamic
      } else {
        val.project_type = "project-x"; // Need to get dynamic
      }
  ...
  addTimeWorkedRow(firstRow): void {
    let defaultVal = '0.00';
    if (this.config.hours_input_type?.type !== HoursInputType.DECIMAL) {
      defaultVal = HoursDefaultValues[this.config.hours_input_type?.format];
    }
    let row = { projectCode: null, total: defaultVal, totalDecimal:0.00 ,
    account_code: null, duplicate: false, oldVal:null,
    projectInfo: {account_code: null,title: null, type: 'rate_factor'},
    project_id: null, project_title: null, project_type: 'default'
  };
    for (let i = 0; i < this.weekDates.length; i++) {
      row[this.weekDates[i].day] = defaultVal;
      row[this.weekDates[i].dayNumber] = null;
  ...
  calculateTotals() {
    ...
    this.formData.grandTotalDecimal = (this.formData?.grandTotalDecimal);
  ...
  validateProjectCodes() {
    ...
    let isValid = true;
    this.formData.rowData.forEach(row => {
      if (!row.projectCode && row?.project_type != 'default') {
        isValid = true;
        this.projectRequiredCheck = false;
      } else {
  ...
  validateDuplicateProjectCodes() {
    ...
    this.formData?.rowData?.forEach((row: any, i) => {
      ...
      this.formData?.rowData?.forEach((dup, j) => {
        if ((i != j && (row?.project_id == dup?.project_id) && (row?.project_title == dup?.project_title)) && (row?.project_type != 'default' && parseInt(row?.total) != 0)){
          isValid = false;
          row.duplicate = true;
        }
      });
  ...
  copyTimesheetInSelectedWeek(event) {
    ...
    if (event?.timesheet) {
      // Copy Timesheet
      event?.timesheet?.data?.forEach(item => {
        item?.data?.forEach((data, index) => {
          data.date = this.timesheetService.formatDateInRequiredFormat(this.weekDates[index]?.date, "yyyy-MM-dd");
        });
      });
      // End
      this.populateAccountCodes();
