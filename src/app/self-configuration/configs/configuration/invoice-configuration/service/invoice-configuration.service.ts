...
export class InvoiceConfigurationService {
  ...
  getInvoiceDropdownOptions() {
    return {
      invoiceCode: [ { value: this.currentProgram?.code, label: this.currentProgram?.code },],
      voucherCode: [{ value: 'VCN', label: 'VCN' },],
      batchCode: [ { value: this.currentProgram?.code, label: this.currentProgram?.code },],
  ...
  createConnectionToS3FileServer(params) {
    return this.post(`/configurator/file-uploader`, { params })
  }
  ...
  getconfigpickList(){
    let url =`/invoice/programs/${this.currentProgram?.id}/config-picklist`;
    return this.get(url).pipe(takeUntil(this.$destroy));
  }
