...
export class CreateRFxComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('rfxIframe', { static: false }) rfxIframe: ElementRef<HTMLDivElement>;

  eventSubscriber: Subscription
  iframeInstance: HTMLIFrameElement
  ...
  rfxURL(base: boolean) {
    let query = ""
    if (base) {
      query = environment.RFX_URL + "/rfx/create";
    } else {
      if (this.router.url.includes('/rfx/create')) {
        query = environment.RFX_URL + "/rfx/create";
  ...
  IframeOnload(iframe: HTMLIFrameElement, src: string, reset = false) {
  ...
  SendCredentials(iframe: HTMLIFrameElement, src: string, reset = false) {
