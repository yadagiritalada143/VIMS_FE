...
@Component({
  selector: 'app-expression-builder-condition',
  templateUrl: './expression-builder-condition.component.html',
  styleUrls: ['./expression-builder-condition.component.scss']
})
export class ExpressionBuilderConditionComponent implements OnInit {
  @Input() propertyList;
  @Input() condition;
  @Input() fullcondition
  @Input() outerIndex;
  ...
  ngOnInit(): void {
    ...
    this.property_options = this.propertyList?.map(x => {
      ...
      if(x.config?.nest_value) {
        x.field.nest_value = x.config?.nest_value ? true : false
      }
      if(x.config?.append_path_from_child) {
        x.field.append_path_from_child = true
        if(x.children) {
          x.field.children = x.children[0]
        }
      }
      if(x.config?.display_name){
        x.name =  x.config?.display_name;
        x.field.name = x.config?.display_name
      }
      if(x?.children?.length && x?.children[0]?.config?.display_name){
        x.field.nestValuePlaceholder = x.children[0]?.config?.display_name // Override value of field display name if we have 'x.children[0]?.config?.display_name' this field.
      }
      return x
    })
    this.property_options = this.sortPipe.transform(this.property_options, 'name');
    if(this.isEditMode) {
      this.allSelectedValues = this.condition.inner_condition.map(x=>x.selectedProperty + x.selectedValue).flat();
      this.allBooleanSelectedValues = this.condition.inner_condition.filter(f=>f.fieldType == "BOOL").map(x=>x.selectedProperty + (x.selectedCustomField ? x.selectedCustomField : '')+ x.selectedValue)
  ...
  searchValues(term, innerIndex, inner_group,modulename?) {
    ...
    if(eventData) {
      if(modulename == 'customfield' || (modulename == 'selectedvalue' && (!eventData?.field?.field_meta?.rendering?.outer_key.includes('foundational_data_types')))){
        if(this.condition.inner_condition[innerIndex]?.searchableName) {
          this.condition.inner_condition[innerIndex].searchableName = null
        }
        if((eventData?.field?.slug == "custom_field" && modulename == 'customfield') || (eventData?.field?.slug != "custom_field")) {
  ...
  getValuesByProperty(eventData,innerIndex, term?) {
    if(eventData) {
      this.condition.inner_condition[innerIndex].operatorList = eventData?.field?.field_operators ?? this.condition.inner_condition[innerIndex].operatorList;
      this.condition.inner_condition[innerIndex].fieldType = eventData?.field?.field_type;
      this.condition.inner_condition[innerIndex].isRequiered = eventData?.config?.required ? true : false;
      this.condition.inner_condition[innerIndex].nestValuePlaceholder = eventData?.field?.nestValuePlaceholder ? eventData?.field?.nestValuePlaceholder : 'Please Select'
      if (eventData?.field?.field_type?.toLowerCase() === 'dropdown') {
        if(eventData?.config?.static_dropdown) {
          let idArray = eventData?.field?.field_meta?.rendering?.display_key_map.value || eventData?.field?.field_meta?.rendering?.display_key_map.id;
          let nameArray = eventData?.field?.field_meta?.rendering?.display_key_map.display_text || eventData?.field?.field_meta?.rendering?.display_key_map.name;
          let tempData= [];
          for(let i=0; i<eventData?.field?.field_meta?.rendering?.outer_key.length; i++){
            tempData = tempData[eventData?.field?.field_meta?.rendering?.outer_key[i]]
          }
          this.condition.inner_condition[innerIndex].valueList = this.getArrangedData(tempData,idArray,nameArray)
        } else {
          let url = eventData?.field?.data_source?.api_url;
          ...
          if (term || term == '') {
            ...
            if((term || term == '') && url.includes('?')) {
              eventData?.field?.field_meta?.rendering?.filter_param ? url += `&${eventData?.field?.field_meta?.rendering?.filter_param}=${term}` : url += `&k=${term}`;
          ...
          this.progServ.get(url).subscribe((data: any) => {
            ...
            let keyName = eventData?.field?.field_meta?.rendering?.outer_key;
            let idArray = eventData?.field?.field_meta?.rendering?.display_key_map.value || eventData?.field?.field_meta?.rendering?.display_key_map.id;
            let nameArray = eventData?.field?.field_meta?.rendering?.display_key_map.display_text || eventData?.field?.field_meta?.rendering?.display_key_map.name;
            for(let i=0; i<eventData?.field?.field_meta?.rendering?.outer_key.length; i++){
              data = data[eventData?.field?.field_meta?.rendering?.outer_key[i]]
            }
            ...
            if(this.condition.inner_condition[innerIndex]?.valueList?.length >= 1 && this.condition.inner_condition[innerIndex]?.target_field_obj?.length >= 1 && (this.condition.inner_condition[innerIndex]?.selectedProperty == eventData.id)) {
          ...
          //url includes foundation and source field value then call new
        }
      }
      if(eventData?.field?.field_operators?.length) {
        this.condition.inner_condition[innerIndex].operatorList = eventData?.field?.field_operators
      }
  ...
  getFoundationalDataValuesByProperty(eventData,innerIndex, term?) {
    if(eventData && eventData.children) {
      this.condition.inner_condition[innerIndex].operatorList = eventData?.children[0]?.field?.field_operators ?? this.condition.inner_condition[innerIndex].operatorList;
      this.condition.inner_condition[innerIndex].fieldType = eventData?.children[0]?.field?.field_type;
      this.condition.inner_condition[innerIndex].isRequiered = eventData?.children[0]?.config?.required ? true : false;
      if (eventData?.children[0]?.field?.field_type?.toLowerCase() === 'dropdown') {
        let url = eventData?.children[0]?.field?.data_source?.api_url;
        if(url) {
          ...
          this.progServ.get(url).subscribe((data: any) => {
            ...
            if(this.condition.inner_condition[innerIndex].target_field_obj) {
            ...
            if(this.condition.inner_condition[innerIndex]?.valueList?.length >= 1 && this.condition.inner_condition[innerIndex]?.target_field_obj?.length >= 1) {
              ...
              if(this.condition.inner_condition[innerIndex]?.nestSearchableName) {
                this.valueListLoading = true;
                url = eventData?.children[0]?.field?.data_source?.api_url;
                url = url.replace('<program_id>', this.programDetails?.id);
                url = url.replace('<foundational_data_type_id>', this.condition.inner_condition[innerIndex].selectedCustomField ? this.condition.inner_condition[innerIndex].selectedCustomField : this.condition.inner_condition[innerIndex].source_field_meta);
      ...
      }else if (eventData?.children[0]?.field?.field_type?.toLowerCase() === 'bool' &&  eventData?.config?.parent_dropdown_child_bool) {
        this.condition.inner_condition[innerIndex].selectedValue = this.condition.inner_condition[innerIndex].target_field_value[0]
      }
      if(eventData?.children[0]?.field?.field_operators?.length) {
        this.condition.inner_condition[innerIndex].operatorList = eventData?.children[0]?.field?.field_operators
      }
      if(eventData?.children[0]?.config) {
        this.condition.inner_condition[innerIndex].isValueMultiselected = eventData?.children[0]?.config?.multiselect ? true : false
      }
