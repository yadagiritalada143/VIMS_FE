...
@Component({
  selector: 'app-all-progress-update',
  templateUrl: './all-progress-update.component.html',
  styleUrls: ['./all-progress-update.component.scss']
})
export class AllProgressUpdateComponent implements OnInit {
  ...
  sowURL(base: boolean) {
    let query = ""
    if (base) {
      query = environment?.SOW_URL + "/progress_list";
    } else {
      if (this.router?.url?.includes("/sow/progress_list?")) {
        query = environment?.SOW_URL + (this.router?.url).replace('/sow/progress_list?', '/progress_list?');
      }
  ...
  IframeOnloadEvent(event: MessageEvent, iframe: HTMLIFrameElement, reset: boolean) {
    ...
    if ((!!(event?.data?.history) || !!(event?.data?.redirect)) == false) {
      ...
      this.location?.go('/progress_list');
    }
