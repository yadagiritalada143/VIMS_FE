...
export class CreateSowsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  sowURL(base: boolean) {
    let query = ""
    if (base) {
      query = environment.SOW_URL + "/create_sow";
    } else {
      if (this.router.url.includes("/sow/") || this.router.url.includes("/vendor_sow/")) {
        if (this.router.url.includes('/sow/create_sow')) {
          query = environment.SOW_URL + "/create_sow";
        } else {
  ...
  IframeOnloadEvent(event: MessageEvent, iframe: HTMLIFrameElement, reset: boolean) {
    ...
    if ((!!(event.data.history) || !!(event.data.redirect)) == false) {
      this.SendCredentials(iframe, this.sowURL(true), true)
      this.location.go('/sow/create_sow');
    }
